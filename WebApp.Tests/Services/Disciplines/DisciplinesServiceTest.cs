...
{
    public class DisciplinesServiceTest : MySqlConnectionFactory
    {
        public void Clear()
        {
            try
            {
                ...
                var com = new MySqlCommand("sp_DeleteAllDisciplines", this.Connection);
        ...
        public async Task<Discipline> Edit(Discipline discipline)
        {
            await this.Connection.OpenAsync();
            ...
            var command = new MySqlCommand($"SELECT * FROM disciplines where id={discipline.Id};",
                this.Connection);
            var reader = command.ExecuteReader();
            var editdiscipline = new Discipline();
            while (reader.Read())
            {
                var id = reader.GetValue(0);
                var name = reader.GetValue(1);
